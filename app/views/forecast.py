...
def _render_config(
    automl_result: dict, has_ensemble: bool = False
    ...
    if has_ensemble:
        all_models = ["ensemble"] + all_models
        best_model = "ensemble"
...
def _render_panel_chart(project_id: str, panel_id: str, forecast_points: list[dict]) -> None:
    ...
    st.plotly_chart(fig, width="stretch", key=f"forecast_chart_{panel_id}")
...
def render() -> None:
    ...
    if forecast_result and not st.session_state.get("forecast_job_id"):
        ...
        st.divider()
        if st.button("↺ Построить новый прогноз", width="stretch"):
    ...
    model_name, horizon, panel_ids = _render_config(automl_result, has_ensemble=has_ensemble)
