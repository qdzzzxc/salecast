...
class TestDecomposeMstl:
    def test_returns_dict(self, seasonal_series):
        result = decompose_mstl(seasonal_series, freq="MS")
        assert isinstance(result, dict)

    def test_has_required_keys(self, seasonal_series):
        result = decompose_mstl(seasonal_series, freq="MS")
        assert "trend" in result
        assert "seasonal" in result
        assert "remainder" in result

    def test_components_sum_to_original(self, seasonal_series):
        result = decompose_mstl(seasonal_series, freq="MS")
    ...
    def test_short_series_returns_trivial(self):
...
class TestExtractSeasonalVectors:
    ...
    def test_vector_length_equals_period(self, panel_df):
        result = extract_seasonal_vectors(
            panel_df, "article", "sales", freq="MS", season_lengths=[12],
        )
    ...
    def test_normalized_vectors(self, panel_df):
        result = extract_seasonal_vectors(panel_df, "article", "sales", freq="MS")
