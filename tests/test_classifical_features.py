...
class TestAddTrendFeatures:
    def test_column_added(self, single_panel_df) -> None:
        result = _add_trend_features(single_panel_df, "sales", window=6)
        assert "sales_trend_6" in result.columns

    def test_no_leakage_shift(self, single_panel_df) -> None:
    ...
    def test_values_are_finite_after_warmup(self, single_panel_df) -> None:
        window = 4
        result = _add_trend_features(single_panel_df, "sales", window=window)
        trend_col = result[f"sales_trend_{window}"]
    ...
    def test_original_df_unchanged(self, single_panel_df) -> None:
...
class TestAddCdfFeatures:
    ...
    def test_values_in_0_1(self, single_panel_df) -> None:
        result = _add_cdf_features(single_panel_df, "sales", decay=0.9)
    ...
    def test_first_value_nan(self, single_panel_df) -> None:
...
class TestBuildTsFeatures:
    def test_trend_disabled_by_default(self, multi_panel_df) -> None:
        settings = Settings()
    ...
    def test_cdf_disabled_by_default(self, multi_panel_df) -> None:
        settings = Settings()
        assert not settings.downstream.use_cdf
        result = build_ts_features(multi_panel_df, settings, disable_tqdm=True)
        assert "sales_cdf" not in result.columns

    def test_trend_enabled(self, multi_panel_df) -> None:
        settings = Settings(downstream=DownstreamConfig(use_trend=True, trend_window=4))
        result = build_ts_features(multi_panel_df, settings, disable_tqdm=True)
        assert "sales_trend_4" in result.columns

    def test_cdf_enabled(self, multi_panel_df) -> None:
        settings = Settings(downstream=DownstreamConfig(use_cdf=True, cdf_decay=0.8))
        result = build_ts_features(multi_panel_df, settings, disable_tqdm=True)
        assert "sales_cdf" in result.columns

    def test_both_features_enabled(self, multi_panel_df) -> None:
        settings = Settings(
    ...
    def test_per_panel_no_leakage_across_panels(self, multi_panel_df) -> None:
    ...
    def test_mstl_seasonal_disabled_by_default(self, multi_panel_df) -> None:
        settings = Settings()
        assert not settings.downstream.use_mstl_seasonal
        result = build_ts_features(multi_panel_df, settings, disable_tqdm=True)
        assert "sales_mstl_seasonal" not in result.columns

    def test_mstl_seasonal_enabled(self, multi_panel_df) -> None:
        settings = Settings(downstream=DownstreamConfig(use_mstl_seasonal=True))
        result = build_ts_features(multi_panel_df, settings, disable_tqdm=True)
    ...
    def test_rename_build_ts_features(self, multi_panel_df) -> None:
