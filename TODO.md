# TODO — Salecast

Живой список задач по проекту. Приоритет сверху вниз внутри каждого раздела.

---

## 🔴 Критично — баги моделирования

### 1. Разрывы на границах train/val/test
**Симптом:** первая точка val/test на графике резко падает вниз относительно реального ряда.
**Вероятные причины:**
- Скейлинг: scaler fit на train, поэтому первое предсказание val скейлится по-другому
- Off-by-one: предсказание строится для `last_train + 1`, но ряд может иметь пропуск дат
- Lag-фичи: лаг-фичи на границе используют нули (expanded panel) вместо реальных значений

**Что проверить:**
- `src/data_processing.py` — как применяется `expand_to_full_panel` и fillna перед фичами
- `src/classifical_features.py` — какие лаги строятся на первых точках val/test
- `src/catboost_utilities/evaluate.py` — как собираются предсказания

**Фикс-гипотеза:** передавать весь ряд (train+val+test) при построении фичей, обрезать по split-границам уже после. Это даст корректные лаги на границах.

---

### 2. Странные предсказания вперёд (forecast)
**Симптом:** прогноз на будущие точки выглядит неадекватно (резкие скачки, нули, константа).
**Вероятные причины:**
- `_forecast_catboost`: рекурсивное добавление предсказанных значений в `running_df` — если первое предсказание плохое, ошибка накапливается
- `build_monthly_features` вызывается на `extended` df, где последние строки имеют `value_col=0.0` — лаговые фичи портятся
- `_forecast_seasonal_naive`: `is_train=False` — проверить что модель корректно смотрит на нужный лаг 12
- StatsModels: `season_length=12` захардкожен — для нечасотных данных неверно

**Что проверить:**
- Написать unit-тест: синтетический ряд с чёткой сезонностью → прогноз должен её воспроизводить
- Проверить `SeasonalNaiveModel.predict` при `is_train=False` — какой индекс используется
- Для CatBoost: попробовать передавать `running_df` с реальными значениями (не 0.0) в value_col при построении фичей (хотя бы для lag_12)

---

## 🟡 Тесты

### 3. E2E тест полного пайплайна через API
Покрыть сценарий:
1. POST `/projects` (загрузить CSV)
2. POST `/projects/{id}/run` (split)
3. Poll `/jobs/{id}` до `status=done`
4. POST `/projects/{id}/run_automl` (models=["seasonal_naive"])
5. Poll до done
6. POST `/projects/{id}/run_forecast` (horizon=3)
7. Poll до done
8. GET `/projects/{id}/forecast_data` → проверить структуру ответа

Файл: `tests/test_e2e_api.py` (pytest + httpx, запускать против живого стека)

### 4. E2E тест моделирования (уже частично есть)
`tests/scripts/test_automl_e2e.py` — расширить:
- Добавить сравнение: прогноз на known holdout vs реальные значения, MAPE должна быть разумной
- Проверить что forecast CSV содержит ровно `n_panels × horizon` строк
- Тест на синтетике с известным паттерном: seasonal naive должен воспроизвести сезонность с MAPE < 15%

---

## 🟡 Надёжность сервисов

### 5. Проверка взаимодействия сервисов (нет мусора)
Проверить что при удалении проекта:
- [ ] Job записи в PostgreSQL удаляются (CASCADE или явно в роутере)
- [ ] Файлы в MinIO удаляются: `projects/{id}/raw.csv`, `splits/`, `automl_predictions/`, `forecast.csv`
- [ ] Redis: ключи Celery результатов протухают (TTL настроен?)

Проверить что при падении воркера в середине задачи:
- [ ] Job переходит в `status=failed` (try/except в каждой задаче — ✅ есть)
- [ ] Не остаётся partial-файлов в MinIO

Проверить что при перезапуске API:
- [ ] Pending job'ы не зависают навсегда (нужен механизм timeout или статус-проверки при старте)

### 6. Очистка MinIO при delete_project
**Сейчас:** `DELETE /projects/{id}` удаляет запись из БД, файлы в MinIO остаются.
**Фикс:** в роутере `projects.py` добавить удаление всего префикса `projects/{id}/`

---

## 🔵 Модели и алгоритмы

### 10. Мультисезонность для дневных данных (MSTL)
**Проблема:** при freq=D сейчас передаётся `season_length=7` — только недельная сезонность.
Дневные данные имеют 3 уровня: 7 (неделя), ~30 (месяц), 365 (год).

**Фикс:**
- Добавить тип модели `mstl` в `StatsForecastModel`:
  ```python
  from statsforecast.models import MSTL, AutoETS
  MSTL(season_length=[7, 365], trend_forecaster=AutoETS())
  ```
- Для D: `season_length=[7, 365]`, для W: `[52]`, для MS: `[12]`
- Требует ≥ 2–3 года данных для надёжной оценки 365-периода

---

## 🟢 Качество и UX

### 7. Проверить Optuna (hyperopt) в продакшне
- [ ] Убедиться что `tune_catboost` не падает при `use_hyperopt=True` через реальный UI (end-to-end)
- [ ] Проверить что прогресс в UI не зависает пока идёт hyperopt (он блокирующий внутри worker'а — таймаут 300s может не хватить при большом n_trials)

### 8. Ноутбуки — убрать лишнее, оставить суть
Сейчас в `notebooks/` много воды: промежуточные выводы, экспериментальные ячейки, тяжёлые outputs.
- [ ] Пройтись по всем ноутбукам из `notebooks/niches/` (0–6)
- [ ] Очистить все outputs (`nbconvert --clear-output` или вручную)
- [ ] Убрать мусорные/экспериментальные ячейки, оставить только суть пайплайна
- [ ] Добавить markdown-заголовки где их нет (что делает этот ноутбук, какой вход/выход)
- [ ] Проверить что ноутбуки запускаются от начала до конца без ошибок (на demo-данных)

### 9. README.md
Сделать в самом конце, когда всё остальное готово.
Структура:
- Краткое описание + скриншоты
- Архитектура (схема сервисов)
- Быстрый старт: `docker compose up`
- Описание пайплайна (Upload → Quality → AutoML → Forecast)
- Используемые технологии

---

## 🟡 UI — мелкие проблемы

### 12. После завершения AutoML исчезает вкладка «Качество данных»
**Симптом:** когда AutoML завершился и результат записан в `session_state.current_project`,
вкладка «Качество данных» перестаёт что-либо показывать.
**Вероятная причина:** `current_project` обновляется с `{...result, ...job["result"]}` — возможно,
ключи `diagnostics` / `ts` / `split` перезаписываются или теряются при мердже словарей.
**Что проверить:** в `app/views/automl.py` строка `new_result = {**result, **job["result"]}` —
убедиться что вложенные ключи `diagnostics` и `ts` не затираются ключами AutoML-результата.

### 13. YAML import/export выглядит плохо
**Симптом:** кнопки «Загрузить» и «Скачать» расположены в двух колонках над формой —
выглядит неорганично, upload занимает слишком много места (drag-and-drop зона).
**Идеи для улучшения:**
- Перенести в `st.popover` или `st.sidebar`-секцию, чтобы не загромождать основной экран
- Или использовать компактный layout: кнопка скачивания как иконка, загрузка через `st.popover`
- Или разместить оба элемента внизу формы после кнопки запуска, как вторичные действия

### 14. Настройки есть только для CatBoost и AutoARIMA
**Отсутствуют настройки для:**
- **AutoETS** — параметр `model` (например `"ZZZ"`, `"AAA"`) и `season_length` override
- **AutoTheta** — `decomposition_type` (`"multiplicative"` / `"additive"`)
- **Seasonal Naive** — нечего менять (период берётся из ts_config), но можно показать
  текущий `season_length` как информационное поле (read-only)

**Приоритет:** низкий — AutoETS/AutoTheta редко тюнятся вручную.

---

## ✅ Готово
- Backend: FastAPI + Celery + Redis + PostgreSQL + MinIO
- Streamlit: Upload, Quality, AutoML (Моделирование), Forecast
- AutoML: SeasonalNaive, CatBoost, AutoARIMA, AutoETS, AutoTheta
- Hyperopt (Optuna) — теперь реально вызывается через worker
- Docker Compose с полным стеком
- Настройки моделей в UI (CatBoost: iterations/lr/depth, AutoARIMA: approximation, Hyperopt: n_trials)
- YAML import/export конфига AutoML
