...
### 8. PatchTST
- [ ] **Сезонность не улавливается** — прогноз плоский, не повторяет паттерн. Возможные причины: мало шагов обучения (`max_steps`), модель underfits. Проверить и подобрать дефолты на реальных данных WB. Текущие дефолты (`input_size = 3×season`, `patch_len = season`) добавлены, но не проверены.
- [ ] Проверить автоподбор `input_size`/`patch_len` по `season_length` на реальных данных WB — убедиться что результат лучше фиксированных значений
...
### 11. Скачивание прогнозов со страницы Моделирование
- [ ] На вкладке AutoML (Моделирование) добавить кнопку «Скачать прогнозы» — CSV с предсказаниями всех моделей на test/val по панелям
- [ ] Опционально: скачать графики как PNG (Plotly `fig.write_image` или через `st.download_button`)
