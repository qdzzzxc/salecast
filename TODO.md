...
### TS2Vec loss chart — сохранение после обучения
- [ ] Сейчас loss chart TS2Vec виден только в процессе обучения (через Redis Stream progress). После завершения — недоступен.
- [ ] Сохранять loss history (epoch → loss) в результат модели и отображать на экране AutoML по клику / в результатах.
