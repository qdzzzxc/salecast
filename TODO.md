...
## 🔴 Глава 2 ВКР — ближайшие задачи

### Кросс-валидация (запуск + раздел 2.6)
- [ ] Запустить CV на WB зеркалах: модель TS2Vec clustered, 3 фолда
- [ ] Запустить CV на Kaggle Store Item Demand: модель CatBoost clustered, 5 фолдов
- [ ] Сохранить скриншоты результатов (таблица фолдов + box plot) → `assets/`
- [ ] Написать раздел 2.6 в `papers/chapter2.md` по результатам CV

### Графики предсказаний для диплома
- [ ] WB: сохранить PNG графиков топ-3 и антитоп-3 панелей (Моделирование → Download plot as PNG) → `assets/`
- [ ] Kaggle: аналогично → `assets/`

### Прочее по главе 2
- [ ] Решить с научруком `[❓ Научрук]` плейсхолдеры в 2.1 (ссылка на источник данных)
- [ ] Пересобрать streamlit, проверить кнопку CSV и загрузку YAML конфига
...
### Прогон на данных WB и Kaggle ✅ ГОТОВО
- Все 12 моделей запущены на обоих датасетах
- Результаты записаны в `papers/chapter2.md` (таблицы 2.7, 2.8)
- WB: лучшая Chronos-2 (test MAPE 1.60), Kaggle: лучшая CatBoost clustered (test MAPE 0.05)

### 12. Кросс-валидация итоговой модели ✅ ГОТОВО (код)
